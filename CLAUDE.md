...
- **Berkeley Mono is the desktop UI font, not just the terminal font.** The surfaces it covers: GTK2 (`gtkrc-2.0`), GTK3 (`gtk-3.0/settings.ini`), the GNOME/GTK4 gsettings keys set by `linux/setup.sh:apply_gsettings_theme` (`font-name`, `document-font-name`, `monospace-font-name`), rofi, dunst, hyprlock's clock and date, Hyprland's groupbar and `misc` font, and every `Text`/`Label` in the Quickshell QML (checked by counting text elements against `font.family` declarations per file, so none falls through to the Qt default). Three things deliberately stay off it: the **four** Quickshell `font.family` lines that draw Nerd Font icons (`BarItem`'s bar icon, `MenuRow`'s row icon and checkmark, `AudioMenu`'s mute/volume glyph) keep FiraCode Nerd Font, since Berkeley Mono has no Private Use Area glyphs and would render tofu; fontconfig's generic `sans-serif`/`monospace` aliases in `linux/configs/fontconfig/fonts.conf` stay on Inter and FiraCode Nerd Font as the fallback for apps and web pages asking for generic sans, where a monospace face reads badly; and Firefox is left alone entirely (Inter for its chrome via `userChrome.css`, Adwaita Sans for web content via `user.js`). Note this means the GTK keys and the fontconfig sans alias intentionally disagree — they drive different consumers. **Bar icons use one uniform `iconSize: 21`** from `BarItem.qml`; the old per-item overrides (28/26/27/24/24) were removed after screenshotting the running bar showed they over-compensated and made the row read as unaligned. Monospace sets roughly 20% wider than Inter per character, so bar labels are 16px and `horizontalPadding` is 7, and the clock format uses single spaces around its `·` (double spaces become full-width cells). The weather label strips the trailing unit letter (`+9°C` → `+9°`) because the degree sign otherwise takes its own cell and reads as `+9° C`; `weatherColor()` still reads the unit off the untouched `weatherTemp` property. Verify bar changes by restarting `qs` and screenshotting with `grim` — the differences are optical and do not show up in the diff.
