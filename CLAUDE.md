...
- **Current suspend/lid behavior**: systemd-logind uses its default lid-close suspend policy. Hypridle locks before suspend but has no `after_sleep_cmd`: a post-resume `hyprctl` DPMS dispatch caused `start-hyprland` to abort after an otherwise clean s2idle resume. The historical workaround note below is superseded and retained only as investigation history.
