...
- **There is no Ghostty terminal server, and no systemd setup for Ghostty at all.** It used to run one — Fedora's `app-com.mitchellh.ghostty.service` plus a drop-in carrying `--gtk-single-instance --quit-after-last-window-closed=false --background-opacity=0.92` — so `$mod+Return` asked a resident process for a window (~250ms) instead of cold-starting one (574ms). Two measurements retired it. kitty opens a fresh process in **0.24s**, beating the server's **0.62s** with no daemon at all; and the server was where Ghostty's leak compounded, going 69 → 204 MB across ~5 window open/close cycles and holding that with **zero windows open**. The unit is Fedora's, ships disabled, and nothing in this repo enables it, so a fresh bootstrap needs no action — the drop-in, its `MANAGED_LINKS` entry and the `enable_ghostty_server`/`disable_ghostty_server` step are all gone. One thing worth remembering if a server is ever wanted again: **a single-instance client forwards only the request, so config flags passed to it are silently dropped** (`--background-opacity`, `--title`, `--class` all no-op) — which is why that opacity had to live on the *server* command line. The `$mod+Return` Ghostty bind deliberately omits `--gtk-single-instance` for the same reason the server is gone: that flag would make the first window the instance owner for every later one, reinstating the shared-process accumulation.
- **Ghostty is the default terminal; kitty is kept installed as the fallback.** `$mod+Return` opens Ghostty, `$mod+SHIFT+ALT+Return` opens kitty, and `linux/scripts/docker-menu.sh` shells out with `ghostty -e`. kitty held the default for a stretch on the strength of a memory benchmark, and that benchmark still stands as the known caveat rather than a reason to switch back: Ghostty 1.3.1 leaks on **window lifecycle**, not throughput — a synthetic 120k-line unicode burst (the trigger in upstream #10244) moved it 204 → 204 MB, while opening and closing ~5 windows took it 69 → 204 MB and it held that with zero windows open, matching #9433 rather than #10244. Over a day of real use it reached 1068 MB with 964 MB of private dirty heap against a 10 MB `scrollback-limit`, so scrollback never explained it. Upstream also tracks #9786 (Ghostty + Claude Code specifically) and #10269 (still leaking on 1.3.0-main after a PageList fix); kitty's fresh window peaked at 162 MB on the same payload and returned all of it on exit. That is exactly why **Ghostty is launched WITHOUT `--gtk-single-instance`, and why no Ghostty server is enabled**: one process owning every window is what makes the leak permanent, while separate processes hand the memory back on close — the same reasoning that retired the server above. The kitty fallback is launched without `--single-instance` for the same reason. `shared/configs/kitty/.config/kitty/kitty.conf` mirrors the Ghostty config (VSCode Dark palette, FiraCode Nerd Font Retina 15, ligatures off via `features="-liga -calt"`), and **neither config has split or tab bindings** — tmux owns that layer and its panes survive the terminal dying, which terminal-native splits do not. The Linux-only 0.92 opacity lives in a gitignored `local.conf` beside each config, *not* a launch flag: a flag only reaches windows opened by that one keybind, leaving rofi, `docker-menu.sh` and `.desktop` launches at the macOS value (Ghostty 0.58, kitty 0.72). Ghostty pulls its copy in with `config-file = ?local.conf` as the **last** line of its config — the `?` makes the include optional so it is a no-op on macOS, a relative path resolves against the config file's own directory, and last value wins so the line has to stay at the bottom; kitty uses `globinclude local.conf`. Both are written by `linux/setup.sh` (`write_ghostty_local_conf` / `write_kitty_local_conf`). The Ghostty config also pins `gtk-single-instance = false`, so a flagless launch (the keybind, `docker-menu.sh`, scripts) always gets its own process rather than trusting Ghostty's `detect` default — but **that does not reach rofi or `.desktop` launches**: Fedora's packaged entry carries `Exec=ghostty --gtk-single-instance=true`, an explicit CLI flag beats the config file (measured — two launches share one pid with the flag, separate pids without), so those still join one long-lived server that nothing reaps. The leak is current on 1.3.1-4.fc44: ~10 MB per window open/close, 231 MB held with zero windows open, and the default `quit-after-last-window-closed = true` does not fire in single-instance mode. Fixing the launcher path means shadowing the desktop entry in `~/.local/share/applications`, deliberately not done — it costs those launches ~270ms → ~680ms and needs re-checking on package updates. Cold start itself is not tunable: renderer (`GSK_RENDERER` ngl/gl/vulkan), font, `GTK_A11Y`, shell-integration and even `--config-default-files=false` all measure 640–690ms, against kitty's 223ms and a single-instance client's 274ms. **The terminal font is FiraCode Nerd Font, Retina weight (450), size 15** (the pre-Berkeley reference config restored from `ca7fd1b^`, with the weight switched to Retina). The configs are split by backend capability: on Linux the patched Nerd Font build is kitty's text font directly (fontconfig handles it, icons come built in at natural width), while **on macOS kitty cannot use the non-Mono patched Nerd Font build as its text font** — kitty's CoreText backend silently rejects the non-Mono builds (their double-width icon glyphs fail its monospace validation) and falls back to Menlo with no warning, and its `family=`/`style=` matching against Fira Code's non-RIBBI weights is also broken (every style resolves to the Light face). So `macos.conf` overrides the font block with plain Fira Code pinned by `postscript_name=`, and `macos.conf`'s `symbol_map` sends the Private Use Area to the standalone Symbols Nerd Font (brew cask `font-symbols-only-nerd-font`); Ghostty uses the patched build on both platforms without issue. Verify resolved faces with `ghostty +show-face --string=Ag` and `kitty --debug-font-fallback`. **Berkeley Mono remains installed** (paid, per-user licensed — PRIVATE `vishnukg/fonts` repo, synced into gitignored `shared/fonts/` by `shared/lib/fonts.sh`; macOS copies the OTFs via `ensure_local_fonts`, Linux links them into `~/.local/share/fonts`) and was the terminal font for a stretch; if it returns, its faces need `postscript_name=` addressing plus the `linux/configs/fontconfig/fonts.conf` `spacing=100` scan rule (still in place) — only the Regular OTF declares monospace spacing, so without that rule fontconfig hides the other faces from kitty entirely. Full history in commits `ca7fd1b`/`94986b9`/`8615032`. **Note the SUPER binds in both terminal configs are macOS-only in practice** — Hyprland claims `SUPER+h/j/k/l`, `SUPER+1..9` and `SUPER+Return` first, so they never reach the terminal on Linux.
