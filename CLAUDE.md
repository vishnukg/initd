...
2. `<platform>/managed-links.sh` appends OS-only entries (currently nothing on macOS; hypr, hyprmoncfg, quickshell, rofi, dunst, fontconfig, gtk-3.0, the `initd-hyprland-session.service` user unit and the three `night-light*` user units — plus, conditionally, `shared/fonts/berkeley-mono` when present — on Linux). Fonts are deliberately NOT linked on macOS: CoreText refuses to register a font reached through a symlink, so `macos/bootstrap.sh:ensure_local_fonts` copies them instead.
...
- **Speaker amplifiers** (`linux/setup.sh:enable_xps13_sidecar_amps`): the XPS 13 DX13260 (DMI SKU 0E53) drives its speakers through two Cirrus CS35L56 "sidecar" amps on SPI beside the cs42l43 codec, and `sof_sdw` only wires them into the card when the `SOC_SDW_SIDECAR_AMPS` quirk (BIT(16) = 65536) is set. Upstream adds this SKU in commit `efd80de2de9d`, first shipped in Linux 7.2-rc5; on older kernels SOF logs "No SoundWire machine driver found", the amps bind but never join the card (no `AMPL`/`AMPR Speaker` mixer controls — Cirrus names them that, not cs35l56 — and firmware `patched=0`) and the codec's tiny built-in amp drives the speakers alone — thin, with a harsh 3–6 kHz peak. The fix is the one Omarchy ships as `dell-xps13-sidecar-amps`: `options snd_soc_sof_sdw quirk=65536` in `/etc/modprobe.d`. The module is not in Fedora's initramfs so no dracut rebuild is needed, only a reboot. Verified working on 7.1.12: `Overriding quirk 0x1 => 0x10000`, both amps load `cs35l56-b2-dsp1-misc-10280e53-spkid1.wmfw` plus tuning, and `amixer -c 0 controls | grep AMP` lists the two amplifiers. The step is gated on product name + SKU and self-retires: when the running kernel's module already carries the `Dell XPS WCL` quirk string it removes the drop-in. The earlier 6-band PipeWire filter-chain EQ (`sink-eq6.conf`, `enable_audio_eq`, the `~/.config/pipewire` link) was tuned against the codec-only path and has been removed — git history has it; if the amped output still wants voicing, build a new curve against the real path (Omarchy's `omarchy-audio-tuner` documents a measured workflow) rather than restoring the old one. `AudioMenu.qml` still filters filter-chain nodes out of the picker, which is the right default for any future tuning sink. **Speaker DRC is off** (`linux/setup.sh:disable_speaker_drc`): Intel's SOF topology enables a dynamic-range compressor on the speaker pipeline (`Post Mixer Speaker Playback DRC switch`) by default. It is a generic loudness leveller rather than a tuning for this laptop, and it was what buried quieter instruments under the vocal; the Cirrus firmware still does speaker protection, so it is safe off. It is a plain ALSA mixer control, but Fedora's `90-alsa-restore.rules` re-applies `/var/lib/alsa/asound.state` to the card on every boot, so the step also runs `alsactl store` and only reports done when the live value and the stored file agree. The IIR/FIR EQ bytes controls on the same pipeline are left at their defaults; if more presence is ever wanted, tune it by ear (EasyEffects) and bake the result into a PipeWire filter chain rather than poking those blobs.
