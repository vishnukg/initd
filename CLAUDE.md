...
- **kitty is the default terminal; Ghostty is kept installed.** `$mod+Return` opens kitty, `$mod+SHIFT+ALT+Return` opens Ghostty. The switch is a memory decision: Ghostty 1.3.1 leaks unboundedly under heavy terminal output — measured ~1 GB over 24 h on one window here, with 964 MB of private dirty heap against a 10 MB `scrollback-limit`, so scrollback cannot explain it. Upstream tracks it in ghostty-org/ghostty discussions #9786 (specifically Ghostty + Claude Code), #10244 (non-ASCII output) and #10269 (still leaking on 1.3.0-main after a PageList fix). Reclaiming it means `systemctl --user restart app-com.mitchellh.ghostty.service`, which closes every window at once. `shared/configs/kitty/kitty.conf` is a faithful translation of the Ghostty config — same VSCode Dark palette, Fira Code Medium 15, splits and tab binds — with the differences noted inline where kitty has no equivalent (`toggle_background_opacity` becomes two binds; `goto_window` walks tab windows, not OS windows). **kitty is launched WITHOUT `--single-instance`**, unlike the Ghostty server: one process owning every window is precisely what made that leak session-wide, and separate processes return memory to the OS on close. The Linux-only 0.92 opacity rides on `-o background_opacity=` at launch, the same split the Ghostty setup uses, so the shared config keeps macOS's 0.58 and is never edited for Linux reasons. **Note the SUPER binds in both terminal configs are macOS-only in practice** — Hyprland claims `SUPER+h/j/k/l`, `SUPER+1..9` and `SUPER+Return` first, so they never reach the terminal on Linux.
