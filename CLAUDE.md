...
- **System fixes** that need sudo: `video` group membership (backlight is `root:video`; required for the XF86MonBrightness keybinds — takes effect after re-login), disabling unused `ModemManager` (no modem on this hardware), the lid-lock-only override described below. These are applied by `linux/setup.sh`.
