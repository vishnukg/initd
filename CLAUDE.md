...
linux/bootstrap.sh
  ...
  ├─ install_packages                          # enables sdegler/hyprland and
...
- **Monitors**: `hyprmoncfg` owns monitor profiles, workspace assignments, hotplug, and lid changes. Its generated `monitors.lua` is loaded by `pcall(require, "monitors")` in `hyprland.lua`; profiles are versioned under `linux/configs/hyprmoncfg/`. See `docs/linux-monitors.md`.
