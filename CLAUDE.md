...
- **Hyprland config** (`linux/configs/hypr/hyprland.conf`): per-monitor scale (laptop panel 1.25x, externals 1.25x); `switch:on:Lid Switch` binds handle lid events; built-in blur/rounding/animations; `kb_options = ctrl:nocaps` and `repeat_delay/rate` for input. The polkit auth agent is `lxpolkit` (Fedora has neither `polkit-gnome` nor a `hyprpolkitagent` package).
...
- **System fixes** that need sudo: swappiness, `video` group membership (backlight is `root:video`; required for the XF86MonBrightness keybinds — takes effect after re-login). These are applied by `linux/setup.sh`.
