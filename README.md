...
| `~/.config/systemd/user/hyprmoncfgd.service.d` | `linux/configs/systemd/user/hyprmoncfgd.service.d` (Hyprland-only condition for the packaged daemon) |
...
Intel video decoding is configured by `ensure_video_codecs()` in `linux/bootstrap.sh`. It enables RPM Fusion free/nonfree repositories, replaces Fedora's restricted Intel VA-API driver with `intel-media-driver`, and installs `ffmpeg-free`, `libavcodec-freeworld`, and `libva-utils` for H.264/HEVC support. To apply just this step to an existing Fedora install, run `bash -c 'source linux/bootstrap.sh; ensure_fedora; ensure_video_codecs'` from the repo. Restart Firefox afterward, then check `vainfo` and the codec table in `about:support`; package installation alone does not prove that a playing video uses hardware decoding.

`linux/setup.sh` handles things that don't fit the symlink flow:
- system fixes requiring sudo (`video` group membership for backlight keys; disabling idle daemons this machine doesn't use: `ModemManager`, Fedora's `abrt` crash reporter, `rsyslog`)
