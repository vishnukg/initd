...
write_ghostty_local_conf() {
  # Linux runs Ghostty at 0.92 while the shared config ships macOS's 0.58. It
  # has to be a config file rather than a `--background-opacity=` flag: Ghostty
  # is launched from several places (the $mod+Return bind, rofi's .desktop
  # entry, docker-menu.sh) and only the keybind could carry a flag, so the
  # others would silently render at the macOS value.
  local target="${ROOT_DIR}/shared/configs/ghostty/.config/ghostty/local.conf"
  ...
  if [[ -f "${target}" ]]; then
    log_success "Ghostty local.conf already present."
    return
  fi

  cat > "${target}" <<'EOF'
# Machine-local Ghostty overrides. Generated by linux/setup.sh; gitignored.
# Pulled in by the `config-file = ?local.conf` line at the end of the shared
# config, which carries macOS's values; Linux-only ones belong here.
background-opacity = 0.92
EOF
  log_success "Ghostty local.conf written (Linux opacity 0.92)."
}

write_kitty_local_conf() {
  # Same split for the kitty fallback ($mod+SHIFT+ALT+Return): a config file
  # rather than a `-o background_opacity=` flag, so a kitty window opened from
  # rofi matches one opened from the keybind.
  local target="${ROOT_DIR}/shared/configs/kitty/.config/kitty/local.conf"

  # The directory is tracked (kitty.conf lives in it), but mkdir -p keeps this
  # from being the one step that fails on a partial or hand-made checkout.
  mkdir -p "$(dirname "${target}")"

  if [[ -f "${target}" ]]; then
    log_success "kitty local.conf already present."
    return
  fi

  cat > "${target}" <<'EOF'
# Machine-local kitty overrides. Generated by linux/setup.sh; gitignored.
# The shared kitty.conf carries macOS's values; Linux-only ones belong here.
background_opacity 0.92
EOF
  log_success "kitty local.conf written (Linux opacity 0.92)."
}
