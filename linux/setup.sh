...
write_kitty_local_conf() {
  # Linux runs kitty at 0.92 while the shared config ships macOS's 0.58. It has
  # to be a config file rather than a `-o background_opacity=` flag: kitty is
  # launched from several places (the $mod+Return bind, rofi's .desktop entry,
  # docker-menu.sh) and only the keybind could carry a flag, so the others would
  # silently render at the macOS value.
...
apply_gsettings_theme() {
  ...
  gsettings set org.gnome.desktop.interface font-name "Berkeley Mono 11"
  gsettings set org.gnome.desktop.interface document-font-name "Berkeley Mono 12"
