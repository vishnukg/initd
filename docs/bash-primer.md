...
| `tests/shared/install.test.mjs` | Behavior tests for installation, backups, cleanup, and the bootstrap launcher. |
...
OS-only) and re-run `node --test tests/shared/install.test.mjs`.
...
1. **To add a new managed config:** add one line to the appropriate
   ...
   `<platform>/managed-links.sh` for OS-only) and re-run `node --test tests/shared/install.test.mjs`.
